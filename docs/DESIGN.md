...
- *Painted-region grid.* Used whenever a region has actually been painted, in place of overhang sampling and grid projection. Each edge-connected painted patch is cut into horizontal bands at exactly the vertical pitch and each band's surface contour is walked at even arc length, so rows are even in Z (the axis that matters most on a wall) and the spacing across a row follows the surface — which is what covers a swept or tapered face no lattice fits. A patch shorter than one row pitch is an underside and gets an XY lattice clipped to that patch. Painting is an explicit instruction, so this strategy ignores the overhang angle and the contact-face angle filter, and it works patch by patch rather than by casting rays up: several painted faces stacked in one XY column all receive contacts. Islands and minima still run and are deduplicated against the grid, so the rows stay intact. Vertical and horizontal pitch are separate settings.
