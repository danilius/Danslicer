...
## SpaceMouse continuous rotation follow-up — in progress
Clean base433e4f8bea4675788e8914cbd2589acf0a412de8, same ed62/codex/ui-refresh-05. User reports continuous rotation when returning from editor. Prior fix recreated the COM connection at each handoff and accepted its initial deflection immediately. New SixAxisSession retains one driver connection until the final viewport detaches, routes polling only to its owner and requires a neutral reading (configured deadzone) at startup/handoff before accepting motion. Repeated same-owner activation does not interrupt held movement. Late old-window detach cannot release the new owner. No change to sensitivity/poll cadence or held-motion behavior after neutral.
Two deterministic regressions pass, including1000 repeated held/stale rotation polls after editor close, neutral recovery and legitimate continuous held input. Native harness now asserts driver object identity across editor/main in addition to connection availability. Native run in progress; physical motion remains unverified. Full suite running; no rendering/UI range changes, no merge/06/07.

### Continuous-rotation fix verified
New session regression tests pass; final full suite1013 passed, zero failures/skips. Release solution build passed. Native workspace harness exited0 with workspace-ok.txt and no error; installed driver available in editor and after returning to main. New assertions verify the exact same driver object survives both handoffs, with only the active viewport holding a polling reference. Physical cap motion remains untested; synthetic tests specifically cover cached/held rotation on editor close and release-to-neutral recovery. Rendering unchanged; prior GL evidence retained without rerun.
