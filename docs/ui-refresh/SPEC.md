...
Latest SpaceMouse correction: retain one driver session across main/editor focus handoffs and dispose it only when the final viewport detaches. A newly active viewport waits for the cap to return within the configured deadzone before accepting motion. This supersedes reconnect-on-each-handoff behavior; unchanged sustained deflection is valid once neutral has been observed. Buttons queued for the previous owner are drained at handoff.
